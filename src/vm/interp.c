...
/* Automatically generated from Squeak on 17 January 2010 2:40:27 pm 
   by VMMaker 3.11.12
 */
...
sqInt extraVMMemory;
const char *interpreterVersion = "Squeak3.8.1 of '28 Aug 2006' [latest update: #6747]";
...
register struct foo * foo = &fum;
	...
	if (
# ifdef SQ_HOST32  // cast to int for 64 bit image on 32 bit host
		(((((int) (eventTypeIs))) ^ ((((int) (eventTypeIs))) << 1)) >= 0)
# else
		(((eventTypeIs) >= -1073741824) && ((eventTypeIs) <= 1073741823))
# endif  // SQ_HOST32
	) {
		longAtput((arg + BaseHeaderSize) + (0 << ShiftForWord), ((eventTypeIs << 1) | 1));
	} else {
		/* begin primitiveFail */
		foo->successFlag = 0;
	}
...
register struct foo * foo = &fum;
	...
	/* begin pop:thenPush: */
	oop = positive64BitIntegerFor(ioMicroSeconds());
