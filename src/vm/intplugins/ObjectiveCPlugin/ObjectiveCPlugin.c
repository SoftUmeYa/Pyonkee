...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"ObjectiveCPlugin 16 March 2010 (i)"
#else
	"ObjectiveCPlugin 16 March 2010 (e)"
 #endif
 ;
...
EXPORT(sqInt) primitiveGetAddressFromEntryPointString(void) {
	...
	entryPointNameLength = interpreterProxy->slotSizeOf((oopForPointer( aEntryPointName ) - 4));
...
return null;
	...
	classNameLength = interpreterProxy->slotSizeOf((oopForPointer( aClassName ) - 4));
	;
...
return null;
	...
	selectorLength = interpreterProxy->slotSizeOf((oopForPointer( aSelectorName ) - 4));
	;
...
EXPORT(sqInt) primitiveNSInvocationSetStringType(void) {
	...
	stringSize = interpreterProxy->slotSizeOf((oopForPointer( aValue ) - 4));
...
sqInt _return_value;
	...
	byteArraySize = interpreterProxy->slotSizeOf((oopForPointer( aValue ) - 4));
