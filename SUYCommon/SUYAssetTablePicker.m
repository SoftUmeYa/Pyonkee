...
@interface SUYAssetTablePicker ()

@property (nonatomic, assign) int columns;

@end

@implementation SUYAssetTablePicker
...
{
    for (ELCAsset *elcAsset in self.elcAssets) {
        id<ELCAssetDelegate> par = elcAsset.parent;
        elcAsset.parent = nil;
        if (asset != elcAsset) {
...
- (BOOL)shouldDeselectAsset:(ELCAsset *)asset
{
    ...
    if ([self.parent respondsToSelector:@selector(shouldDeselectAsset:previousCount:)]) {
        return [self.parent shouldDeselectAsset:asset previousCount:selectionCount];
    }
    return YES;
}
